...
| The IPv6 side of the same, only while the interface holds a global IPv6 address: the v6 router, the built-in anchors' IPv6 twins (`2606:4700:4700::1111`, `2001:4860:4860::8888`, `2620:fe::fe`), and the first hops of a `traceroute` toward the probe's v6 twin | continuously | is IPv6 configured here, does the router carry it, does the route beyond | ICMPv6 echo; TCP handshakes to the twins' port 443 |
...
| `1.1.1.1:443` and, on a dual-stack link, `2606:4700:4700::1111:443` (QUIC) | once after startup / network change | path-MTU probe, per family (Linux) | padded QUIC version-negotiation packets |
| `api.ipify.org`, and `api64.ipify.org` over IPv6 only (`public_ip6_url`) | once at startup and on network change | discover your public IPv4 and IPv6 to add as targets | none (a GET) |
...
| `github.com:22`, `smtp.gmail.com:25/465/587`, `imap.gmail.com:993`, `cloudflare.com:80/443`, `1.1.1.1:53/853/443` (`egress_checks`); on a dual-stack link also `cloudflare.com:80/443`, `time.cloudflare.com:123` and `2606:4700:4700::1111:53/443` over IPv6 | only when you press `c` | which ports this network lets out, per family | a TCP handshake or one datagram; nothing further |
